...
const ManageWaiters = () => {
  ...
  const initialWaiters = [
    { id: 1, name: 'Michael Scott', email: 'michael.scott@example.com', address: '123 Dunder St', password: 'dundermifflin' },
    { id: 2, name: 'Pam Beesly', email: 'pam.beesly@example.com', address: '456 Dunder St', password: 'paper' },
  ...
  const handleSubmit = (e) => {
    ...
    if (editMode && editWaiterId !== null) {
      ...
      const updatedWaiters = waiters.map(waiter =>
        waiter.id === editWaiterId ? { ...waiter, name: waiterName, email: waiterEmail, address: waiterAddress, password: waiterPassword } : waiter
      );
